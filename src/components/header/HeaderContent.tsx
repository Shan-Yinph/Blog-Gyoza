...
function HeaderMenu({ isBgShow }: { isBgShow: boolean }) {
  ...
  return (
    <nav
      className={clsx('relative inline-flex rounded-full group pointer-events-auto duration-200 mx-auto w-max', {
    ...
    >
      <div
        ...
        aria-hidden
      ></div>
      <div className="text-sm px-4 flex justify-center items-center">
        {menus.map((menu) => (
          <HeaderMenuItem
            key={menu.name}
            href={menu.link}
